...
class FileStore:
    """Small subject-scoped workspace file store backed by the local filesystem."""
    ...
    def _path(self, subject_id: str, name: str) -> Path:
        ...
        path = self._root / subject_id / relative
    ...
    def list(self, subject_id: str) -> list[dict[str, Any]]:
        base = self._root / subject_id
        if not base.exists():
            return []
        return [self._metadata(subject_id, path) for path in sorted(base.rglob("*")) if path.is_file()]
