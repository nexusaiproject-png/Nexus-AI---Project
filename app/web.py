...
@media(max-width:520px){.grid,.tools{grid-template-columns:1fr}.top{align-items:flex-start;flex-wrap:wrap}.status{font-size:11px}.messages{min-height:240px}.msg{max-width:90%}}
</style></head>
...
<main><div class="top"><div><h1 class="title">Nexus AI Workspace</h1><p class="sub">One responsive workspace for your agents, tools and daily work.</p></div><div style="display:flex;gap:12px;align-items:center"><div class="status"><i class="dot"></i>Backend connected</div><button class="secondary" id="logout">Log out</button></div></div>
...
async function requireAuth(){try{const r=await fetch('/auth/me');if(r.status===401){window.location.href='/login';return false}if(!r.ok)throw new Error('auth check failed');return true}catch(_){window.location.href='/login';return false}}
async function loadTools(){try{const r=await fetch('/tools');if(!r.ok)throw new Error('tools unavailable');const d=await r.json();document.getElementById('tool-count').textContent=d.tools.length;document.getElementById('tools').innerHTML=d.tools.map(t=>`<div class="tool"><b>${escapeHtml(t)}</b><span>Available</span></div>`).join('')}catch(e){document.getElementById('tool-count').textContent='offline';document.getElementById('tools').textContent='Unable to load tools.'}}
document.getElementById('logout').onclick=async()=>{await fetch('/auth/logout',{method:'POST'});window.location.href='/login'};
...
(async()=>{if(await requireAuth())loadTools()})();
</script></body></html>"""
